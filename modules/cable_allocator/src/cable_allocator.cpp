...
bool cable_allocator::add_cable(
...
auto cable_allocator::get_cable_allocations(int cable_ID)
    -> std::set<std::tuple<
        AllocationScore,
        std::set<int>>> const
{
...
void cable_allocator::confirme_allocation(
    int cable_ID,
    int allocation_idx)
{
    if (cable_ID < 1 || cable_ID > _cable_list.size())
        ...
        _cable_list.at(cable_ID - 1)
            .confirme_allocation(cable_ID, allocation_idx);
}

auto cable_allocator::get_connector_status()
