...
class AWG
{
    ...
    /**
     * @brief
     * Constructor with integer, if the integer is invalid, set
     * value to unknown.
     */
    explicit constexpr AWG(int gauge)
        : _value(
              is_valid(gauge) ? static_cast<awg_values>(gauge)
                              : awg_values::AWGUnknown)
    {}
     ...
     */
    constexpr operator awg_values() const
    {
        return _value;
    }
     ...
     */
    constexpr bool operator==(const AWG& other) const
    {
