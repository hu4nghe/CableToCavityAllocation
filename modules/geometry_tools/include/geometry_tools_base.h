 ...
 * @brief
 * This file defines serveral basical structure for geometry
 * calculations
 * @version 3.0
 * @date 2026-04-01
 *
...
namespace geo_tools
{
    ...
    static auto dbl_eq =
        [](double a,
           ...
           double epsilon = std::numeric_limits<double>::epsilon())
    {
        auto [diff, relative_epsilon] =
            get_dbl_delta(a, b, epsilon);
    ...
    struct point
    {
        ...
        bool operator<(const point& p) const
        {
            return dbl_eq(x, p.x) ? dbl_inf(y, p.y)
                                  : dbl_inf(x, p.x);
        }
        bool operator>(const point& p) const
        {
    ...
    struct triangle
    {
        ...
        circle circum_circle() const
        {
            double a1 = b.x - a.x, b1 = b.y - a.y,
                   c1 = (a1 * a1 + b1 * b1) / 2, a2 = c.x - a.x,
                   b2 = c.y - a.y, c2 = (a2 * a2 + b2 * b2) / 2,
                   d = a1 * b2 - a2 * b1;
