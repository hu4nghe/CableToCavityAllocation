...
concept electronic_component_data_type =
    (is_wire_component<T> &&
     std::is_same_v<tuple_type, std::tuple<int, int>>) ||
    (is_cavity_component<T> &&
     std::is_same_v<
         tuple_type,
         std::tuple<int, int, double, double>>);

template <typename T> class electronic_container_base
{
     ...
     * Automatically determines the type of component based on the
     * the parameter type.
     *
     * @param components A vector containing all components's data.
     */
    template <typename tuple_type>
        requires electronic_component_data_type<
            T,
            tuple_type>
    electronic_container_base(
        const std::vector<tuple_type>& components)
    {
        for (const auto& p : components)
            _container.push_back(
                std::make_shared<T>(std::make_from_tuple<T>(p)));
    }
    ...
    std::shared_ptr<T> get_component(const int& ID) const
    {
        auto iter = std::ranges::find(
            _container,
            ID,
            &electronic_component_base::get_ID);
     ...
     */
    auto insert(
        container_t::const_iterator pos,
        std::shared_ptr<T>          ptr)
    {
        return _container.insert(pos, ptr);
    }
};