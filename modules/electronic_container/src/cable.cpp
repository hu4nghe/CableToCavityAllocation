...
namespace
{
    // Keep only high-quality candidates to reduce both memory and
    // UI noise.
...
bool cable::generate_allocations(int mode)
{
    ...
    if (auto sp_connector = _wp_connector.lock())
    {
            ...
            [&](const std::vector<int>& visited)
        {
            ...
            if (!_allocations.empty())
            {
                const double best_score =
                    _allocations.begin()->get_score().combined();
                const double cand_score =
                    candidate.get_score().combined();

                // Reject candidates that are much worse than the
                // current best.
            ...
            // Keep only top-k candidates (set is already sorted
            // best -> worst).
            ...
            if (!_allocations.empty())
            {
                ...
                while (!_allocations.empty())
                {
                    ...
                    if (worst_it->get_score().combined() <=
                        new_best * kScorePruneFactor)
                        break;
        ...
        {
            if (mode == 1)
                // Search only the places near the allocated
                // cavities.
                return std::make_shared<connector>(
                    *sp_connector
                    ...
                    | std::views::transform(
                          [&](const auto& obj)
                          {
                              return sp_connector->get_adj_list(
                                  obj->get_ID());
                          })
                    ...
                    // Build filter result(searching near the
                    // allocated cavities.)
                    | std::ranges::to<connector>());

            else return sp_connector;
        };
        ...
        for (const auto& path_head_cavity : *searching_range)
        {
            ...
            auto dfs =
                [&](auto&& self, int cavity_ID, int wire_index)
            {
                auto current_cavity =
                    sp_connector->get_component(cavity_ID);
                if (current_cavity->status())
                    return; // Cavity is already occupied
                auto current_wire = _container[wire_index];
                if (!current_cavity->is_compatible(*current_wire))
                    return; // cavity not compatible

                // Log the searching path
                visited_cavity_indices.push_back(cavity_ID);

                if (visited_cavity_indices.size() ==
                    size()) // Save valid complete regions
                    try_store_candidate(visited_cavity_indices);

                else // Continue searching through available
                     // neighbors
                    for (const auto& neighbor :
                         sp_connector->get_adj_list(cavity_ID))
                        if (!std::ranges::contains(
                                visited_cavity_indices,
                                neighbor))
    ...
    for (const auto& [wire_gauge, num_wires] : wires)
        for (auto i = 0; i < num_wires; ++i)
            _container.emplace_back(
                std::make_shared<wire>(wire_index++, wire_gauge));
}

void cable::confirme_allocation(
    int current_cable_ID,
    int allocation_ID)
{
    ...
    if (auto sp_connector = _wp_connector.lock())
        for (const auto& chosen_cavity_ID : layout)
            sp_connector->get_component(chosen_cavity_ID)
                ->allocate_to_cable(current_cable_ID);
